...
module AXILStore #(
    ...
    parameter NUMBER_READ_WORDS = SoftwareInterfacePkg::FULl_CONFIG_SIZE_IN_WORDS + 4,
...
// Config Ports
  output logic [NUMBER_WRITE_WORDS - 1:0] [31:0] inputs,
  input wire [NUMBER_READ_WORDS - 1:0] [31:0] outputs,
...
// writing operands
///////////////////////////////////////////////////////////////////////////
localparam TOTAL_WIRE_REGISTER = NUMBER_WRITE_WORDS;
logic [$clog2(TOTAL_WIRE_REGISTER) -1:0] write_addr;
logic mask_disable;
...
begin
    if(axil_awvalid)
    begin
        mask_disable <= 0;//~(axil_awaddr >= START_OFFSET & (axil_awaddr <= (START_OFFSET + ((TOTAL_WIRE_REGISTER) <<2))));
        write_addr <= axil_awaddr[2+:$clog2(TOTAL_WIRE_REGISTER)];
    end
end
else
begin
...
begin
  ...
  begin
    if (write_addr < TOTAL_WIRE_REGISTER)
        begin
            for ( integer byte_index = 0; byte_index <= 3; byte_index = byte_index+1 )
                ...
                begin
	              ...
	              inputs[write_addr][(byte_index*8) +: 8] <= axil_wdata[(byte_index*8) +: 8];
	            end
        end
  end
end
...
///////////////////////////////////////////////////////////////////////////
localparam TOTAL_READ_REGISTER = NUMBER_READ_WORDS;
logic [$clog2(TOTAL_READ_REGISTER) - 1:0] read_addr;
...
begin
  read_enable <= axil_arvalid;
    if(axil_arvalid)
    begin
        read_mask_disable <= 0;//~(axil_awaddr > START_OFFSET & (axil_awaddr < (START_OFFSET + ((TOTAL_READ_REGISTER) <<2))));
        read_addr <= axil_araddr[2+:$clog2(TOTAL_READ_REGISTER)];
    end
end
else
begin
...
begin
  ...
  begin
     ...
     begin
      if (read_addr < TOTAL_READ_REGISTER)
         axil_rdata <= outputs[read_addr];
      else
         axil_rdata <= 0;
     end
  end
