...
impl LFSR16 {
    pub fn new(seed: u16,sequence_size: i32) -> Self {
        Self { state: seed, size: sequence_size}
    }
}
...
impl Iterator for LFSR16 {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.size -= 1;
        let new: u16 = get_tap_16(self.state,15) ^ get_tap_16(self.state,13) ^ get_tap_16(self.state,12) ^ get_tap_16(self.state,10);
        let old = self.state;
...
mod tests {
    ...
    fn max_length_test_8() {
        let seed: u8 = 45;
        let mut lfsr8 = LFSR8::new(seed, 256);
    ...
    fn max_length_test_16() {
        let seed: u16 = 45;
        let mut lfsr16 = LFSR16::new(seed, (1 << 16) + 1);
